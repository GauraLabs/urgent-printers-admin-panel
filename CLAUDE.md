...
- Partially wired (printing queue): `getPrintingQueueAll` and `startPrinting` wired to real backend (`GET /admin/orders?status=` × 3 + per-order `GET /admin/orders/{id}` for item-level rows; `PATCH /admin/orders/{id}/status` for the printing transition). `approveArtwork`, `requestReupload`, `markReadyToDispatch`, `getServiceabilityBulk`, `createShipmentsBulk` remain mock — no backend endpoint exists for staff-side artwork approve/reject (only customer-token/WhatsApp-driven `resolve_approval`), no `ready_to_dispatch` OrderStatus, and no courier/shipment provider. See `src/lib/api/printingQueue.ts` comments for exact gaps.
